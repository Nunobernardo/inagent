    ...
    try {
      for (var _iterator = types[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        if (/^\./.test(t)) {
          ...
          if (filename.split('.').pop() === t.replace('.', '').toLowerCase()) return true;
        } else if (/\/\*$/.test(t)) {
...
var FileDropzone = function () {
  ...
  createClass(FileDropzone, [{
    ...
    value: function disableClick() {
      this.element.removeClass('dropzone-area dropzone--clickable');
      this.clickable = false;
    }
  }, {
    key: 'enableClick',
    value: function enableClick() {
    this.element.addClass('dropzone-area dropzone--clickable');
      this.clickable = true;
    }
  }], [{
    key: 'getFileSize',
